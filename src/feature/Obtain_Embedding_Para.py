...
def main():
    ...
    parser.add_argument("--embedding_type", default="para_embedding", type=str)
    ...
    if args.year == "2019":
        ...
        if args.dataset_type == 'review':
            ...
            for d in tqdm(data):
                ...
                articles = []
                for r in d['reviews']:
                    clean_review = clean_text(r['review'])
                    articles.append(clean_review)
                    embeddings = model.encode(clean_review, device=device)
            ...
            for d in tqdm(data):
                ...
                human_articles = clean_text(d['meta_reivew']["comment"])
                ...
                gpt_articles = clean_text(d['meta_review_gpt'])
                ...
                gemini_articles = clean_text(d['meta_review_gemini'])
                ...
                claude_articles = clean_text(d['meta_review_claude'])
        ...
        if args.dataset_type == 'review':
            ...
            for d in tqdm(data):
                ...
                for r in d['review_human']:
                    review = clean_text(r['review'])
                    human_articles = review
                    human_embeddings = model.encode(human_articles, device=device)
                ...
                gpt_articles = clean_text(d['review_gpt'])
                gpt_embeddings = model.encode(gpt_articles, device=device)
                save_data['GPT4o'].append(
                    {
                        'reviews_sent': gpt_articles,
                ...
                gemini_articles = clean_text(d['review_gemini'])
                gemini_embeddings = model.encode(gemini_articles, device=device)
                save_data['Gemini'].append(
                    {
                        'reviews_sent': gemini_articles,
                ...
                claude_articles = clean_text(d['review_claude'])
                claude_embeddings = model.encode(claude_articles, device=device)
                save_data['Claude'].append(
                    {
                        'reviews_sent': claude_articles,
